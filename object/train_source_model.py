...
def test_target(args):
    ...
    if args.da == 'oda':
        acc_os1, acc_os2, aAcccc_unknown = cal_acc_oda(dset_loaders['test'], netF, netB, netC)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Code for Domain Adaptation*',
    ...
    args = yaml.load(open(".\object\office-train-config.yaml"))

    save_config = yaml.load(open(".\object\office-train-config.yaml"))
    ...
    #将模型首次训练完之后需要将该模型与其他的域进行一个首次的域适应，初始效果需要保存，之后进行对比
